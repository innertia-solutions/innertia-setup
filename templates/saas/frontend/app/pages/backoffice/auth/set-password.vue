...
async function handleSubmit() {
  ...
  try {
    await api.post('backoffice/auth/password/set', {
      token: token.value,
      email: email.value,
  ...
  <div class="space-y-6">
    ...
    <div v-if="invalidLink" class="flex flex-col items-center justify-center py-10 space-y-4 text-center">
      ...
      <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs">
        Este enlace de activación no es válido o ha expirado. Contacta al administrador.
      </p>
      <NuxtLink to="/backoffice/login" class="text-sm text-blue-600 hover:underline dark:text-blue-400">
        Volver al inicio
      </NuxtLink>
    </div>
    ...
    <div v-else-if="success" class="flex flex-col items-center justify-center py-10 space-y-4 text-center">
      ...
      <h2 class="text-xl font-bold text-slate-800 dark:text-slate-200">¡Cuenta activada!</h2>
    ...
    <template v-else>
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Crea una contraseña para completar el registro.
        </p>
      </div>
      ...
      <form class="space-y-4" @submit.prevent="handleSubmit" novalidate>
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              data-hs-toggle-password='{"target": "#set-password"}'
